import { RTM, Store, Until, RTC, liveCode } from "../../../utils/arcall/index";
Page({
  /**
   * 页面的初始数据
   */
  ...
  onLoad: function (options) {
    ...
    Until.on("livePusherUrlEvent", this, (data) => {
      _this.setData({
        livePusherUrl: data.livePusherUrl
          ? data.livePusherUrl
          : _this.data.livePusherUrl,
        livePlayerUrl: data.livePlayerUrl
          ? data.livePlayerUrl
          : _this.data.livePlayerUrl,
      });
    });
    // 通话模式变更
    ...
    // 检测远端用户音视频状态
    Until.on("peerAudioVideoStatus", this, (data) => {
      _this.setData({
